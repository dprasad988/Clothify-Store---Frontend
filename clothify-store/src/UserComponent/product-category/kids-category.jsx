...
function KidsCategory() {
    ...
    const kidsProducts = products.filter(product => 
      Array.isArray(product.categoryName) && product.categoryName.includes("Kids")
    );

    // Get unique subCategoryName values
    useEffect(() => {
      const uniqueSubCategories = [
        ...new Set(kidsProducts.map((product) => product.subCategoryName)),
      ];
      ...
      <div className="mb-4">
        ...
        <h5 style={{ color: 'white', textAlign: 'center' ,  fontSize: isMobile ? "15px" : "20px"}}>
            <span style={{ borderBottom: '2px solid white', paddingBottom: '5px' }}>
                CHECK OUT THE CLOTHIFY KID'S COLLECTION!
            </span>
        </h5>
        </div>
      </div>
      ...
      <Row
        ...
        >
          ...
          <Col xs={24} sm={8} md={6}>
            ...
            >
              {Array.from(
                new Set(kidsProducts.map((product) => product.brand))
              ).map((brand) => (
